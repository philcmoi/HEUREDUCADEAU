...
if ($pdo) {
    try {
        ...
        if (!$erreur_bdd) {
            ...
            // ==============================================
            // RÉCUPÉRATION DES IMAGES ET CALCUL DES PROMOTIONS
            // ==============================================
            $produits_js = [];
            $images = [];
...
<head>
    ...
    <style>
        ...
        * {
        ...
        /* ==============================================
           STYLES SPÉCIFIQUES AU CATALOGUE AVEC PROMOTIONS
           ============================================== */
        ...
        @media (max-width: 992px) {
            ...
            .catalogue-header h1 { font-size: 2.2rem; }
        ...
        @media (max-width: 768px) {
            ...
            .catalogue-header { padding: 30px 0; }
            ...
            <?php else: ?>
                ...
                <div class="product-card" data-id="<?= $produit['id_produit'] ?>">
                    ...
                    <div class="product-info">
                        ...
                        <!-- Affichage du prix avec ou sans promotion -->
                        ...
                        if (this.cartDropdownItems && data.panier && data.panier.length > 0) {
                            ...
                            for (const item of data.panier) {
                                itemsHtml += `
                                    <div class="cart-dropdown-item" data-id="${item.id_produit}" data-item-id="${item.id_item}">
                                        <div class="cart-dropdown-item-image">
                                            <img src="${item.image || 'https://via.placeholder.com/300x300/95a5a6/ffffff?text=Produit'}" 
                                                 alt="${item.nom}"
                                                 onerror="this.src='https://via.placeholder.com/300x300/95a5a6/ffffff?text=Produit'">
                                        </div>
                                        <div class="cart-dropdown-item-info">
                                            <h4>${item.nom.substring(0, 30)}${item.nom.length > 30 ? '...' : ''}</h4>
                                            <div class="cart-dropdown-item-details">
                                                <span class="cart-dropdown-item-price">${parseFloat(item.prix_unitaire).toFixed(2).replace('.', ',')} €</span>
                                                <span class="cart-dropdown-item-quantity">x${item.quantite}</span>
                                            </div>
                                        </div>
                                        <div class="cart-dropdown-item-remove" onclick="panierManager.supprimerDuPanier(${item.id_produit}, ${item.id_item}, event)">
                                            <i class="fas fa-times"></i>
                                        </div>
                                    </div>
                                `;
