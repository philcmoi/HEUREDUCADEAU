...
// Si panier existe en session mais pas en BDD, le migrer
if (isset($_SESSION['panier']) && !empty($_SESSION['panier']['items']) && !isset($_SESSION['panier_id'])) {
    try {
        // Créer un nouveau panier en BDD
        $sqlCreate = "INSERT INTO panier (id_client, session_id, statut, date_creation) 
                     VALUES (?, ?, 'actif', NOW())";
        $stmtCreate = $pdo->prepare($sqlCreate);
        $stmtCreate->execute([$client_id, session_id()]);
        $panier_id = $pdo->lastInsertId();
        $_SESSION['panier_id'] = $panier_id;
        
        // Migrer chaque item du panier session vers BDD
        foreach ($_SESSION['panier']['items'] as $key => $item) {
            // Vérifier que le produit existe et est actif
            $sqlCheck = "SELECT id_produit, prix_ttc, quantite_stock 
                        FROM produits 
                        WHERE id_produit = ? AND statut = 'actif'";
            $stmtCheck = $pdo->prepare($sqlCheck);
            $stmtCheck->execute([$item['id_produit']]);
            $produit = $stmtCheck->fetch(PDO::FETCH_ASSOC);
            
            if ($produit) {
                // Vérifier le stock disponible
                $quantite = min($item['quantite'], $produit['quantite_stock']);
                
                if ($quantite > 0) {
                    // Insérer dans panier_items
        ...
        $stmtUpdate->execute([$panier_id]);
        
        // Optionnel : vider le panier session après migration
        // $_SESSION['panier'] = ['items' => [], 'count' => 0, 'total' => 0];
        
        error_log("Panier migré de session vers BDD. ID: " . $panier_id);
        
    } catch (Exception $e) {
        error_log("Erreur migration panier: " . $e->getMessage());
        // Continuer avec le panier session en cas d'erreur
    }
}

// ==============================================
// GESTION DU PANIER (identique à avant)
// ==============================================
$panier_id = null;
...
if (!$panier_id) {
    try {
        ...
        $sqlCreatePanier = "INSERT INTO panier (id_client, session_id, statut, date_creation) 
                           VALUES (?, ?, 'actif', NOW())";
...
if ($panier_id) {
    // Récupérer les produits du panier depuis la table panier_items
    $sqlPanier = "SELECT 
                    pi.id_item,
                    pi.id_produit,
                    pi.quantite,
    ...
    // Synchroniser la session avec les données BDD
    $_SESSION['panier'] = [
        'items' => $itemsPanier,
        'count' => $totalArticles,
        'total' => $panierTotal
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Vérifier d'abord si le panier est vide lors de la soumission POST
    if (empty($itemsPanier)) {
        $errors['general'] = "Votre panier est vide. Veuillez ajouter des produits avant de passer commande.";
    } else {
        // Validation des champs de livraison
        $required_fields = ['nom', 'prenom', 'adresse', 'code_postal', 'ville', 'email'];
        ...
        // Validation du téléphone (maintenant optionnel)
    ...
    if (empty($errors)) {
        try {
            ...
            foreach ($itemsPanier as $item) {
                ...
                $tva = $item['prix_ttc'] * 0.20; // 20% de TVA
            ...
            // 6. Vider le panier (BDD et session)
            ...
            // Marquer le panier comme terminé plutôt que le supprimer
            $sqlUpdatePanier = "UPDATE panier SET statut = 'termine', date_modification = NOW() WHERE id_panier = ?";
            $stmtUpdatePanier = $pdo->prepare($sqlUpdatePanier);
            $stmtUpdatePanier->execute([$panier_id]);
...
<body>
                ...
                <div class="recap-section">
                    ...
                    <div class="form-actions">
                        ...
                        <button
                            ...
                            disabled
                        >
                            <i class="fas fa-shopping-cart"></i> Panier vide
                        </button>
                        <?php endif; ?>
                    </div>
                </div>
    ...
    <script>
        ...
        // Formatage automatique du téléphone (optionnel)
        ...
        document.getElementById('checkoutForm')?.addEventListener('submit', function(e) {
            ...
            // Validation des champs obligatoires (téléphone non inclus)
            const requiredFields = [
                'email', 'nom', 'prenom', 'adresse', 'code_postal', 'ville'
            ];
