...
// Vérifier immédiatement si on peut afficher la page
$canDisplay = isset($_SESSION['panier']) && 
              !empty($_SESSION['panier']['items']) && 
              isset($_SESSION['commande']);
  ...
  <body>
    ...
    <script>
      ...
      async function chargerCommande() {
        try {
          const response = await fetch(
            API_BASE_URL + "/commande.php?action=get_commande"
          );
          const data = await response.json();

          if (data.success) {
            commandeData = data.commande;
            panierData = data.panier;
            ...
            if (parseFloat(data.totaux.total) > 0) {
              initialiserPayPal(data.totaux.total);
            }
          } else {
            // Double sécurité : si l'API échoue malgré la vérification PHP
            console.warn("Commande non trouvée via API, redirection");
      ...
      function afficherAdresse() {
        if (!commandeData?.adresse_livraison) return;

        const adresse = commandeData.adresse_livraison;
        const container = document.getElementById("adresseDisplay");

        container.innerHTML = `
                <div style="display: flex; justify-content: space-between; align-items: start;">
                    <div>
                        <p class="adresse-line"><strong>${adresse.prenom} ${adresse.nom}</strong></p>
                        <p class="adresse-line">${adresse.adresse}</p>
                        ${adresse.complement ? `<p class="adresse-line">${adresse.complement}</p>` : ""}
                        <p class="adresse-line">${adresse.code_postal} ${adresse.ville}</p>
                        <p class="adresse-line">${adresse.pays}</p>
                        ${adresse.telephone ? `<p class="adresse-line"><i class="fas fa-phone"></i> ${adresse.telephone}</p>` : ""}
                        <p class="adresse-line"><i class="fas fa-envelope"></i> ${adresse.email}</p>
                    </div>
      ...
      function afficherRecapitulatif() {
        if (!commandeData || !panierData) return;
          ...
          container.innerHTML = `
                    <div style="margin-bottom: 20px;">
                        <p style="color: #718096; font-size: 14px; margin-bottom: 10px;">
                            <i class="fas fa-box"></i> ${panierData.items_count} article(s)
                        </p>
                        <p style="color: #718096; font-size: 14px;">
                            <i class="fas fa-truck"></i> Livraison ${commandeData.livraison.mode}
                        </p>
                        ${commandeData.emballage_cadeau ? `<p style="color: #718096; font-size: 14px;">
        ...
        // Totaux
        if (summary) {
          const sousTotal = parseFloat(panierData.sous_total) || 0;
          const fraisLivraison = parseFloat(commandeData.livraison.frais) || 0;
          const fraisEmballage = parseFloat(commandeData.frais_emballage) || 0;
          ...
          summary.innerHTML = `
                    ...
                    <div class="summary-item">
                        ...
                        <span>${fraisEmballage.toFixed(2).replace(".", ",")} €</span>
                    </div>
      ...
      function initialiserPayPal(total) {
        // Pour la démo, on utilise un client-id test
        // En production, remplacer par votre vrai client-id
        paypal
          .Buttons({
            style: {
              layout: "vertical",
      ...
      async function finaliserCommande(methode, reference) {
        try {
          ...
          const response = await fetch(
            API_BASE_URL + "/commande.php?action=create_commande",
            {
              method: "POST",
              headers: {
          ...
          if (result.success) {
            ...
            window.location.href = result.redirect || "confirmation.html?cmd=" + result.commande.numero;
          } else {
